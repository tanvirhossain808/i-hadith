...
const Book = ({ code, title, hadishNumber, isActive, id }) => {
    ...
    const setActive = () => {
        setData((pre) => {
            return {
                ...pre,
                books: pre.books.map((book) => {
                    return {
                        ...book,
                        isActive: book.id === id

                    }
                    // if (book.id === id) return isActive: true
                })
            }
        })
    }
    return (
        <div className={`flex items-center gap-2 h-[100px] rounded-xl   w-full pl-3 ${(isHover || isActive) && "bg-[#EBFCF6]"} cursor-pointer`} onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)} onClick={() => setActive()}>

            <div className="relative" >
                <h3 className={`text-xl ${(isHover || isActive) ? "text-white" : "text - gray - 400"} absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 xl:text-lg font-medium`}>{code}</h3>
